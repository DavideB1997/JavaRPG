class Prateria extends Phaser.Scene {
	
    constructor() {
        super({ key: 'Prateria' });
		
		this.sceneKey= "Prateria"
    ...
    preload() {
        this.load.image('assets_spritesheet_v2_free', 'assets/tilesets/assets_spritesheet_v2_free.png');
		this.load.image('pokemonerba', 'assets/tilesets/pokemon.jpg');
		
		this.load.image('prato', 'assets/tilesets/tilemap_packed.png');
		
        this.load.tilemapTiledJSON('mappaPrato', 'assets/maps/prateria.json');
    ...
    create(data) {
        ...
        const map = this.make.tilemap({ key: 'mappaPrato' });
		this.map = map;
		
		this.physics.world.setBounds(0, 0, this.map.widthInPixels, this.map.heightInPixels);
		
        const casa = map.addTilesetImage('assets_spritesheet_v2_free', 'assets_spritesheet_v2_free');
		const pokemon = map.addTilesetImage('pokemonerba', 'pokemonerba');
		const prato = map.addTilesetImage('prato', 'prato');
		
		
        this.pavimentoLayer = map.createLayer('pavimento', prato, 0, 0);
		this.casaLayer = map.createLayer("casa",casa,0,0)
		
		this.oggettivari = map.createLayer("oggettivari",casa,0,0)
		this.altrioggettivari = map.createLayer("altrioggettivari",casa,0,0)
		
		this.alberifront = map.createLayer("alberifront",casa,0,0)
		
		this.combatLayer = map.createLayer("erba",pokemon,0,0)
		
		
		
		this.creaTriggerZone("interno",165,250,50,50);
		
		this.creaTriggerZone("dungeon",940,320,50,50);
        ...
        this.collisionLayer = this.casaLayer
		...
		console.log(data.scenaPrecedente )

		if (!data.scenaPrecedente) {
			this.addHero(data.id, 0, 320)
		}
		else if (data.scenaPrecedente == "InternoCasa") {
			this.addHero(data.id, 165, 310)
			data.scenaPrecedente = null;
		}
		else if (data.scenaPrecedente == "Dungeon") {
			console.log("siamo tornati dalla scena dungeon")
			this.addHero(data.id, 890, 310)
			data.scenaPrecedente = null;
		}
	...
	triggerEvent(player, zone) {
		...
		this.autoremoveHero(playerId)
			.then(() => {
				...
				if (zone.nome === "dungeon") {
					this.scene.start('Dungeon', {
						id: playerId, // Passa l'ID del giocatore alla nuova scena
					});
				} else if (zone.nome === "interno") {
					this.scene.start('InternoCasa', {
						id: playerId, // Passa l'ID del giocatore alla nuova scena
					});
				}
		...
		this.autoremoveHero(playerId)
			.then(() => {
				...
				if (zone.nome == "interno") {
					this.scene.start('InternoCasa', {
						id: playerId, // Passa l'ID del giocatore alla nuova scena
					});

				} else if (zone.nome == "dungeon") {
					this.scene.start('Dungeon', {
						id: playerId, // Passa l'ID del giocatore alla nuova scena
					});
				}
			...
			startCombat(hero, tile) {
			    ...
			    if (tileProperties.combat) {
			        ...
			        if (randomValue > 95) {
			            ...
			            this.scene.launch('CombatScene',  { currentScene: 'Prateria', heroId: this.myPlayer.id });
